...
const getPriceObj = (crypto, prices, curr, once) => {
  ...
  const transformObject = (price, conversionFactor, originalDigits) =>
    Object.fromEntries(
      Object.entries(price).map(([_, value]) => [
        _,
        parseFloat((value * conversionFactor).toFixed(originalDigits)),
      ])
    );
  ...
  for (const testCurrency of ["USD", "BTC", "BNB", "ETH"]) {
    [price, supportCurrency] = [
      getPriceObj(crypto, prices, testCurrency, true),
      testCurrency,
    ];
    if (price) break;
  }
  ...
  for (const testCurrency of QUOTES?.[supportCurrency]?.quotes || [
    supportCurrency,
  ]) {
